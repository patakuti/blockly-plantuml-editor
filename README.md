...
- State diagrams have no PlantUML import, and renaming a State/Composite State doesn't update Transition blocks that referenced its old name (unlike Participant/Swimlane renaming in the other two diagram types) — a stale reference is instead flagged with a warning icon.
