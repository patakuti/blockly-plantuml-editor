...
- **State diagrams**: State, Transition, Composite State, Raw PlantUML Line — Transition's from/to fields are dynamic dropdowns listing every declared State/Composite State plus `[*]` (the start/end pseudostate); its label is optional. Composite State groups a sequence of states/transitions under a nested `state Name { ... }` block and can nest arbitrarily deep. Renaming a State or Composite State block automatically updates every Transition field that referenced its old name. Choice pseudostates, concurrent regions (`--`), and history pseudostates aren't supported (use Raw PlantUML Line for those).
- **Notes on any block**: right-click a block and "Add Comment" to attach freeform text; it's rendered as a PlantUML `note` right after that block's own output. A "Note direction" right-click item (shown only on commented blocks) toggles the note between `right` (default) and `left`. On state diagrams, only State and Composite State blocks support notes (rendered as `note X of <name>`, explicitly anchored to that block's own name) — Transition has no "Add Comment" option, since PlantUML has no reliable way to attach a note to a transition.
...
- State/Composite State names must be simple identifiers with no spaces or `"` — PlantUML's state-diagram parser was confirmed (against the public server) to misparse the diagram type entirely, or reject the syntax outright, when a Transition's endpoints are quoted or contain a space. Input isn't validated or sanitized to enforce this; using such a name will just produce a broken PlantUML render.
- Renaming a State/Composite State doesn't update Transition blocks that referenced its old name (unlike Participant/Swimlane renaming in the other two diagram types) — a stale reference is instead flagged with a warning icon.
- Notes are only supported on State and Composite State blocks; Transition has no "Add Comment" menu item, since PlantUML has no reliable syntax for attaching a note to a transition (verified against the public server: the implicit `note right`/`note left` form can fail to render depending on what precedes it).
- choice pseudostates, concurrent regions (`--`), and history pseudostates (`[H]`/`[H*]`) aren't supported by a dedicated block; use Raw PlantUML Line to write that syntax directly.
...
- State diagram `note left/right of <name>` blocks are only reattached as a block comment when they immediately follow that same-named State/Composite State (matching how the generator always places them); otherwise — including a note placed right after a Transition, which can never carry a comment — the note's lines are kept as Raw PlantUML Line blocks instead.
...
- If two Participant blocks happen to share the same name, renaming one of them retargets every Message/Note that referenced that name, even ones that were meant to point at the other, unchanged block (the app doesn't prevent duplicate participant names, and references are matched by name, not by block identity).
