...
- **State diagrams**: State, Transition, Composite State, Raw PlantUML Line — Transition's from/to fields are dynamic dropdowns listing every declared State/Composite State plus `[*]` (the start/end pseudostate); its label is optional. Composite State groups a sequence of states/transitions under a nested `state Name { ... }` block and can nest arbitrarily deep. There's no rename-tracking for this diagram type yet (unlike Participant/Swimlane in the other two), and choice pseudostates, concurrent regions (`--`), and history pseudostates aren't supported (use Raw PlantUML Line for those).
